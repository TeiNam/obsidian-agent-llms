...
このドキュメントは**0.7.14**を対象としています。バージョンごとの変更は[変更履歴](CHANGELOG.md)を参照してください。
...
- **ファイル添付** — ドラッグ＆ドロップ、クリップボード、ファイル検索。`.md`・`.txt`などのテキストファイルは4バックエンドすべてで動作し、ファイルごとに先頭8,000文字まで送信して、切り詰めた場合はプロンプトとファイルチップに表示。画像は4バックエンドすべて、PDFはBedrock・Gemini、OfficeドキュメントはBedrockで動作。非対応の組み合わせは黙って破棄せず、その形式を扱えるバックエンドの一覧とともに拒否する
...
Obsidian 1.13以降の設定検索で現在のバックエンドに表示される設定を探せます。以前のObsidianでは従来の設定画面を利用します。
...
**Bedrock:** AWSコンソール(Bedrock → API keys)で発行した**Bedrock APIキー**を入力し、AWSリージョンを設定します。プラグインはキーを自動更新しないため、期限切れ後は交換してください。

キーの発行、モデルアクセスの確認、複数端末での利用: [Bedrock セットアップガイド](docs/bedrock-setup-ja.md)

> 0.3.0でAWSアクセスキーと`~/.aws`プロファイル(SSO含む)認証を削除しました。このバックエンドは設定に入力したBedrock APIキーを使います。
...
> **キーの保存:** ローカル保存に成功したキーはOSキーチェーンで暗号化され、ボルト同期には含まれません。端末ごとに設定してください。旧`data.json`のキーはローカル保存成功後にのみ除去します。保存に失敗した場合は、再起動前に問題を解消して再保存してください。
...
- 📎 現在のノートを添付
- 🔍 ファイルを検索して添付
- 📁 ファイルピッカー、入力エリアへのドラッグ＆ドロップ、クリップボード貼り付けでPCのテキストファイル(`.md`・`.txt`など)・画像・PDFを添付
...
- **ボルト全体の列挙** — 検索、Graph RAG、Second Brain、ファイル選択にObsidianのファイル一覧APIを使います。索引作成ではノートと対応テキスト添付を分割し、**設定した埋め込みAPIへ送信**します。添付したファイル・ノートやツールで読んだ内容もモデルへのリクエストに含まれる場合があります。ローカルOllama以外の外部エンドポイントでは内容が端末外へ送られます。
- **クリップボード** — コピーボタンで書き込み、チャットへの貼り付け時に読み取ります。書き込み完了後に成功表示を出し、失敗時は通知します。

変更点と検証範囲は[0.7.14レビュー記録（韓国語）](docs/review-0.7.14.md)を参照してください。
