...
export const VIEW_I18N = {
  en: {
    ...
    toolConsecutiveFailures: "Tool execution failed 3 times (in a row or on the same tool). Stopping the tool loop to prevent further errors.",
    toolRoundLimit: (max: number) =>
      `Reached the tool round limit (${max}) before finishing. Send another message to continue.`,
...
${content}`,
  ...
  ko: {
    ...
    toolConsecutiveFailures: "도구 실행이 3회 실패하여(연속 또는 같은 도구) 루프를 중단합니다. 추가 오류를 방지하기 위해 중단되었습니다.",
    toolRoundLimit: (max: number) =>
      `도구 호출 한도(${max}라운드)에 도달해 작업을 마치지 못하고 중단했습니다. 이어서 진행하려면 다시 요청하세요.`,
...
${content}`,
  ...
  ja: {
    ...
    toolConsecutiveFailures: "ツール実行が3回失敗したため(連続または同じツール)、ループを停止します。",
    toolRoundLimit: (max: number) =>
      `ツール呼び出しの上限(${max}ラウンド)に達したため、完了前に停止しました。続けるにはもう一度依頼してください。`,
